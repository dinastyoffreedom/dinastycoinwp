 ...
 * Modified to work with Dinastycoin-wallet-rpc by Dinasty of Freedom trust
 */

defined( 'ABSPATH' ) || exit;

class Dinastycoin_Wallet_Rpc
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
            if(is_admin())
                echo '[ERROR] Failed to connect to dinastycoin-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /*
     * The following functions can all be called to interact with the Dinastycoin RPC wallet
    ...
    public function make_uri($address, $amount, $recipient_name = null, $description = null)
    {
        ...
        $new_amount = $amount * DINASTYCOIN_GATEWAY_ATOMIC_UNITS_POW;
