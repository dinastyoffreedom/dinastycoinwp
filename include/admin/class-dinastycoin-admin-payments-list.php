 ...
 * Authors: mosu-forge
 * modified for Dinastycoin support by Dinasty of Freedom trust
 */
...
class Dinastycoin_Admin_Payments_List extends WP_List_Table {
    ...
    function extra_tablenav($which) {
        if ($which == "top") {
            ...
            if(Dinastycoin_Gateway::get_confirm_type() == 'dinastycoin-wallet-rpc') {
                $balance = Dinastycoin_Gateway::admin_balance_info();
                $balance_info = <<<HTML
...
HTML;
            ...
            echo <<<HTML
            <div class="wrap">
                <h1 class="wp-heading-inline">Dinastycoin Payments</h1>
                $balance_info
                <hr class="wp-header-end">
                <ul class="subsubsub">
                    <li>
                        <a href="?page=dinastycoin_gateway_payments&type=all" {$tab_info['all']['active']}>
                            All <span class="count">({$tab_info['all']['count']})</span>
                        </a> |
                    </li>
                    <li style="display:none">
                        <a href="?page=dinastycoin_gateway_payments&type=pending" {$tab_info['pending']['active']}>
                            Pending <span class="count">({$tab_info['pending']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=dinastycoin_gateway_payments&type=paid" {$tab_info['paid']['active']}>
                            Received <span class="count">({$tab_info['paid']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=dinastycoin_gateway_payments&type=confirmed" {$tab_info['confirmed']['active']}>
                            Confirmed <span class="count">({$tab_info['confirmed']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=dinastycoin_gateway_payments&type=expired" {$tab_info['expired']['active']}>
                            Expired <span class="count">({$tab_info['expired']['count']})</span>
                        </a>
                    </li>
                </ul>
                <form id="dinastycoin-payments-filter" method="get" style="display:none">
                    <p class="search-box">
                ...
                <h2 class="screen-reader-text">Dinastycoin Payments List</h2>
                <style>
...
HTML;
    ...
    public function column_default($item, $column_name) {
        ...
        case 'col_payment_id':
            ...
            break;
        case 'col_txid':
            $url = DINASTYCOIN_GATEWAY_EXPLORER_URL.'/tx/'.$item->txid;
        ...
        case 'col_height':
            ...
            break;
        case 'col_amount':
            echo Dinastycoin_Gateway::format_dinastycoin($item->amount).'Dinastycoin';
            break;
        }
    }
    ...
    public function no_items() {
        esc_html_e('No Dinastycoin payments found', 'dinastycoin_gateway');
    }
